...
for scaling_factor in scaling_factors:
    ...
    for bus_id in bus_i_subset:
        ...
        print("Original load", original_load)
...
print(f"Number of actual Load Points: {len(load_time_series_mapped.columns)}")

# For example, to access the time series for the first load point in the subset:
time_series_load_point_1 = load_time_series_mapped.iloc[:, 37]

# To access the time series for the second load point:
time_series_load_point_2 = load_time_series_mapped.iloc[:,38]
time_series_load_point_3 = load_time_series_mapped.iloc[:,39]
time_series_load_point_4 = load_time_series_mapped.iloc[:,40]

# Print the maximum load demand for each load point
print(f"Maximum load demand in load point 90: \n{max(time_series_load_point_1)}")
print(f"Maximum load demand in load point 91: \n{max(time_series_load_point_2)}")
print(f"Maximum load demand in load point 92: \n{max(time_series_load_point_3)}")
print(f"Maximum load demand in load point 96: \n{max(time_series_load_point_4)}")
